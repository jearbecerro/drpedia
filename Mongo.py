...
def get_terms(users, sender_id):
    return users.find({'user_id': sender_id,'accept_disclaimer':1})
...
def get_ask(users, sender_id):
    return users.find({'user_id': sender_id},{ 'last_message_ask':1 })
...
def get_answer(users, sender_id):
    return users.find({'user_id': sender_id},{ 'last_message_answer':1 })
