...
def create_user(sender_id, user_fb):
    ...
    qry="insert into users (sender_id, created_at, last_seen, first_name, last_name, last_message_ask, last_message_answer, accept_disclaimer) values(?, ?, ?, ?, ?, ?, ?, ?);",(str(sender_id), str(timestamp), "1970-01-01 00:00:00", str(user_fb['first_name']), str(user_fb['last_name']), 'None','None','No',)
    cur.execute(qry)
    con.commit() 
